...
import 'package:lottery_app/detail/ssq_history_detail.dart';
import 'package:lottery_app/master/model/item-model.dart';
import 'package:lottery_app/commons/tools.dart';
import 'package:lottery_app/detail/ssq-detail.dart';
import 'package:lottery_app/commons/constants.dart';
import 'package:lottery_app/commons/app_navigator.dart';
import 'package:lottery_app/master/widgets/ssq_achieve_widget.dart';
import 'package:lottery_app/model/global_config.dart';
import 'package:lottery_app/model/ssq_master.dart';
import 'package:provider/provider.dart';

class SsqMasterPage extends StatefulWidget {
  String _masterId;

  int index;

  SsqMasterPage(this._masterId, {this.index = 6});

  @override
  SsqMasterPageState createState() => new SsqMasterPageState();
}

class SsqMasterPageState extends State<SsqMasterPage> {
  ...
  Widget build(BuildContext context) {
    return AnnotatedRegion<SystemUiOverlayStyle>(
      ...
      child: ChangeNotifierProvider<SsqMasterModel>(
        create: (_) => SsqMasterModel.initialize(masterId: widget._masterId),
        child: Scaffold(
          body: Container(
  ...
  Widget _buildContent() {
    return Consumer2<GlobalConfigModel, SsqMasterModel>(
      builder: (
        BuildContext context,
        GlobalConfigModel global,
        SsqMasterModel model,
        Widget child,
      ) {
              ...
              child: Column(
                children: <Widget>[
                  if (model.detail.vip == 1)
                    IssueNotice(
                      notice:
                          '查看预测需${global.config.ssqFee}金币，最多使用代金券抵扣${global.config.ssqVoucher}金币',
                    ),
            ...
            dragContainer: DragContainer(
                drawer: Container(
                  child: OverscrollNotificationWidget(
                    child: SsqAchieveWidget(record: model.records),
                  ),
  ...
  ///创建专家信息组件
  Widget _buildMaster(SsqMasterDetail detail) {
    return Container(
      color: Colors.white,
      child: Column(
        children: <Widget>[
                ...
                Expanded(
                  child: ListView(
                    ...
                    children: <Widget>[
                      ...
                      _buildRate('25码', detail.red25Hit, 0),
                      _buildRate('杀三', detail.kill3Hit, 0),
                      _buildRate('蓝五', detail.blue5Hit, 0),
  ...
  Widget _buildRate(String name, String hit, double margin) {
    return new Container(
      margin: EdgeInsets.fromLTRB(margin, 0, Adaptor.width(10), 0),
  ...
  Widget _buildHistory(SsqHistory history) {
  ...
  List<Widget> _buildTabPage(SsqHistory history) {
    switch (widget.index) {
      case 1:
        return <Widget>[
          _buildPage(context, history.red25),
          ...
          _buildPage(context, history.blue3, showHit: true),
          _buildPage(context, history.blue5, showHit: true),

...
import 'package:lottery_app/model/ssq_chart.dart';
import 'package:lottery_app/package/vip-package.dart';
import 'package:provider/provider.dart';

class SsqComprehensivePage extends StatefulWidget {
  @override
  SsqComprehensivePageState createState() => new SsqComprehensivePageState();
}
...
class SsqComprehensivePageState extends State<SsqComprehensivePage> {
  @override
  Widget build(BuildContext context) {
    return AnnotatedRegion<SystemUiOverlayStyle>(
      value: SystemUiOverlayStyle.light,
      child: ChangeNotifierProvider<SsqChartModel>(
        create: (_) => SsqChartModel.initialize(),
        child: Scaffold(
          body: Column(
            children: <Widget>[
              MAppBar('综合统计'),
  ...
  Widget _buildList() {
    return Consumer(
        builder: (BuildContext context, SsqChartModel model, Widget child) {
      return Container(
        child: Column(
          children: <Widget>[
            _buildTitle(model),
  ...
  ///页面标题
  ///
  Widget _buildTitle(SsqChartModel model) {
    String title = '';
  ...
  Widget _buildGridTab(SsqChartModel model) {
    return Padding(
  ...
  List<Widget> _createChart(SsqChartModel model, int index) {
  ...
  Widget _buildHelpInfo() {
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          Text(
            '1.综合分析是将收费专家预测数据按红球双胆、三胆、12码、20码、25码、杀三码、杀六码，蓝球三码、蓝球六码、蓝球杀码指标进行的统计分析。'
          ...
          Text(
            '2.这些指标根据评判标准主要分为两大类，一类是统计数值越大越好，如：20码、25码指标；另一类是统计值越小越好，如：红球杀三码、杀六码、蓝球杀码指标。',
            style: TextStyle(
              fontSize: Adaptor.sp(12),
              color: Colors.black38,
            ),

...
import 'package:lottery_app/model/fc3d_chart.dart';
import 'package:lottery_app/model/fc3d_compare.dart';
import 'package:lottery_app/package/vip-package.dart';
import 'package:provider/provider.dart';

class Fc3dComprehensivePage extends StatefulWidget {
  @override
  Fc3dComprehensivePageState createState() => new Fc3dComprehensivePageState();
}
...
class Fc3dComprehensivePageState extends State<Fc3dComprehensivePage> {
  @override
  Widget build(BuildContext context) {
    return AnnotatedRegion<SystemUiOverlayStyle>(
      value: SystemUiOverlayStyle.light,
      child: ChangeNotifierProvider<Fc3dChartModel>(
        create: (_) => Fc3dChartModel.initialize(),
        child: Scaffold(
          body: Column(
            children: <Widget>[
              MAppBar('综合统计'),
  ...
  Widget _buildList() {
    return Consumer(
      builder: (BuildContext context, Fc3dChartModel model, Widget child) {
        return Container(
          child: Column(
            children: <Widget>[
              _buildTitle(model),
  ...
  Widget _buildTitle(Fc3dChartModel model) {
    String title = '';
    if (model.period != null) {
      title = '第${model.period}期福彩3D综合统计';
    }
    return Container(
  ...
  Widget _buildGridTab(Fc3dChartModel model) {
    return Padding(
  ...
  Widget _buildHelpInfo() {
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          Text(
            '1.综合分析是将收费专家预测数据按双胆、三胆、五码、六码、七码、杀一码、杀二码指标进行的统计分析。统计图中四条统计曲线(由下到上)分别对应：排名前10、排名前20、排名前50、排名前100的收费专家统计信息。',
            style: TextStyle(
          ...
          Text(
            '4.关于综合统计指标使用，需要您在使用过程中多观察、多总结，相信一定能为您带来意想不到的收获。',
            style: TextStyle(
              fontSize: Adaptor.sp(12),
              color: Colors.black38,
            ),
        ...
        return Card(
          ...
          child: LineChart(
            data: LineData(
              ...
              datas: [
                _parseData(model.getData().level10),
                _parseData(model.getData().level20),
                _parseData(model.getData().level50),
                _parseData(model.getData().level100),
              ],

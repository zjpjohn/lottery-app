...
import 'package:lottery_app/model/dlt_chart.dart';
import 'package:lottery_app/package/vip-package.dart';
import 'package:provider/provider.dart';

class DltComprehensivePage extends StatefulWidget {
  @override
  DltComprehensivePageState createState() => new DltComprehensivePageState();
}
...
class DltComprehensivePageState extends State<DltComprehensivePage> {
  @override
  Widget build(BuildContext context) {
    return AnnotatedRegion<SystemUiOverlayStyle>(
      value: SystemUiOverlayStyle.light,
      child: ChangeNotifierProvider<DltChartModel>(
        create: (_) => DltChartModel.initialize(),
        child: Scaffold(
          body: Column(
            children: <Widget>[
              MAppBar('综合统计'),
  ...
  Widget _buildCensus() {
    return Consumer(
        builder: (BuildContext context, DltChartModel model, Widget child) {
  ...
  ///页面标题
  ///
  Widget _buildTitle(DltChartModel model) {
    String title = '';
  ...
  Widget _buildGridTab(DltChartModel model) {
    return Padding(
  ...
  ///创建综合分析图表
  ///
  List<Widget> _createChart(DltChartModel model) {
    ...
    if (model.type <= 7) {
      ...
      List<double> level10 = _parseData(model.getData().level10);
      List<double> level20 = _parseData(model.getData().level20);
      List<double> level50 = _parseData(model.getData().level50);
      List<double> level100 = _parseData(model.getData().level100);
      LineChart chart = LineChart(
        data: LineData(
  ...
  Widget _buildHelpInfo() {
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          Text(
            '1.综合分析是将收费专家预测数据按红球双胆、三胆、10码、20码、杀三码、杀六码，蓝球独胆、蓝球双胆、蓝球六码、蓝球杀码指标进行的统计分析。'
          ...
          Text(
            '2.这些指标根据评判标准主要分为两大类，一类是统计数值越大越好，如：红球20码指标；另一类是统计值越小越好，如：红球杀三码、蓝球杀码指标。',
            style: TextStyle(
              fontSize: Adaptor.sp(12),
              color: Colors.black38,
            ),

...
import 'package:lottery_app/model/qlc_overall.dart';
import 'package:lottery_app/components/adaptor.dart';
import 'package:lottery_app/package/vip-package.dart';
import 'package:provider/provider.dart';

class QlcOverallPage extends StatefulWidget {
  @override
  QlcOverallPageState createState() => new QlcOverallPageState();
}
...
class QlcOverallPageState extends State<QlcOverallPage> {
  @override
  Widget build(BuildContext context) {
    return AnnotatedRegion<SystemUiOverlayStyle>(
      value: SystemUiOverlayStyle.light,
      child: ChangeNotifierProvider(
        create: (_) => QlcOverallModel.initialize(),
        child: Scaffold(
          body: Column(
            children: <Widget>[
              MAppBar('综合统计'),
  ...
  Widget _buildCensus() {
    return Consumer(
        builder: (BuildContext context, QlcOverallModel model, Widget child) {
  ...
  Widget _buildTitle(QlcOverallModel model) {
    String title = '';
    if (model.period != null) {
      title = '第${model.period}期七乐彩整体态势';
    }
    return Container(
  ...
  Widget _buildGridTab(QlcOverallModel model) {
    return Padding(
  ...
  ///创建综合分析图表
  ///
  List<Widget> _createChart(QlcOverallModel model) {
  ...
  Widget _buildHelpInfo() {
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          Text(
            '1.综合分析是将平台专家预测数据按红球双胆、三胆、12码、18码、22码、杀三码、杀六码指标进行的统计分析。'
            '统计图中四条统计曲线(由下到上)分别对应：排名前150、排名前300、排名前500、排名前750、排名前1050的收费专家统计信息。',
            style: TextStyle(

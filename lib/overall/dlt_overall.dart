...
import 'package:lottery_app/model/dlt_overall.dart';
...
class DltOverallPage extends StatefulWidget {
  @override
  DltOverallPageState createState() => new DltOverallPageState();
}
...
class DltOverallPageState extends State<DltOverallPage> {
  @override
  Widget build(BuildContext context) {
    return AnnotatedRegion<SystemUiOverlayStyle>(
      value: SystemUiOverlayStyle.light,
      child: ChangeNotifierProvider(
        create: (_) => DltOverallModel.initialize(),
        child: Scaffold(
          body: Column(
            children: <Widget>[
              MAppBar('整体态势'),
  ...
  Widget _buildCensus() {
    return Consumer(
        builder: (BuildContext context, DltOverallModel model, Widget child) {
  ...
  ///页面标题
  ///
  Widget _buildTitle(DltOverallModel model) {
    String title = '';
    if (model.period != null) {
      title = '第${model.period}期大乐透综合统计';
    }
    return Container(
  ...
  Widget _buildGridTab(DltOverallModel model) {
    return Padding(
  ...
  ///创建综合分析图表
  ///
  List<Widget> _createChart(DltOverallModel model) {
    ...
    if (model.type <= 7) {
      ...
      for (int i = 0; i <= 2; i++) {
        ...
        LineChart chart = LineChart(
          data: LineData(
            categories: axis,
            datas: [level150, level300, level500, level750],
      ...
      List<double> level150 = _parseData(model.getData().level150);
      List<double> level300 = _parseData(model.getData().level300);
      List<double> level500 = _parseData(model.getData().level500);
      List<double> level750 = _parseData(model.getData().level750);
      LineChart chart = LineChart(
        data: LineData(
  ...
  Widget _buildHelpInfo() {
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          Text(
            '1.整体态势分析是将平台专家预测数据按红球双胆、三胆、10码、20码、杀三码、杀六码，以及蓝球独胆、三码、六码、杀码指标进行的统计分析。统计图中四条统计曲线(由下到上)分别对应：排名前150、排名前300、排名前500、排名前750、排名前1050的收费专家统计信息。',
            style: TextStyle(
          ...
          Text(
            '2.这些指标根据评判标准主要分为两大类，一类是统计数值越大越好，如：20码指标；另一类是统计值越小越好，如：杀三码、蓝球杀码指标。',
            style: TextStyle(
              fontSize: Adaptor.sp(12),
              color: Colors.black38,
            ),

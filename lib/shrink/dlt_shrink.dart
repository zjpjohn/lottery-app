...
import 'package:lottery_app/shrink/dialog/dlt_lottery_view.dart';
...
import 'package:lottery_app/shrink/model/dlt_shrink_filter.dart';

class DltShrinkPage extends StatefulWidget {
  @override
  DltShrinkPageState createState() => new DltShrinkPageState();
}

class DltShrinkPageState extends State<DltShrinkPage> {
  ///选中的号码
  ///
  Set<int> selects = Set();

  ///选择过滤器
  DltShrinkFilter filter = DltShrinkFilter();
          ...
          _buildShrinkToolItem(
            ...
            callback: () {
              showModalBottomSheet(
                ...
                isScrollControlled: true,
                builder: (BuildContext context) {
                  return SegShrinkWidget(
                    onSelected: (shrink) {
                      setState(() {
                        filter.segFilter = shrink;
                      });
                    },
                  );
                },
                  ...
                  builder: (_) {
                    return Center(
                      child: Material(
                        ...
                        child: DltLotteryView(filter: filter),
                      ),
                    );
                  },
                );
  ...
  Widget _buildSegmentShrink() {
    if (filter.segFilter != null) {
      List<Widget> views = List();
      List<Widget> seg3Views = List();
      filter.segFilter.seg3.entries.forEach((entry) {
              ...
              child: Row(
                children: <Widget>[
                  Container(
                    ...
                    alignment: Alignment.center,
                    child: Text(
                      '${Constant.number2Chinese[entry.key]}区',
                      style: TextStyle(
                        fontSize: Adaptor.sp(12),
                        color: Colors.black38,
                      ),
      ...
      List<Widget> seg4Views = List();
      filter.segFilter.seg4.entries.forEach((entry) {
        if (entry.value.length > 0) {
          seg4Views.add(
            Container(
              margin: EdgeInsets.only(right: Adaptor.width(10)),
      ...
      List<Widget> seg5Views = List();
      filter.segFilter.seg5.entries.forEach((entry) {
        if (entry.value.length > 0) {
          seg5Views.add(
            Container(
              margin: EdgeInsets.only(right: Adaptor.width(10)),

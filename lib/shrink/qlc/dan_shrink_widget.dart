...
class DanShrinkWidget extends StatefulWidget {
  Function(QlcDanShrink) onSelected;
...
///出号数据范围
const List<int> numbers = [1, 2, 3, 4, 5, 6, 7];
...
class DanShrinkWidgetState extends State<DanShrinkWidget> {
  ///选择的胆码
  ///
  QlcDanShrink model = QlcDanShrink();
        ...
        onTap: () {
          setState(() {
            if (model.dans.contains(ball)) {
              ...
              if (model.dans.length >= 10) {
                EasyLoading.showToast('最多选择10个');
                return;
              }
              model.dans.add(ball);
            }

...
import 'package:lottery_app/model/dlt_expert.dart';
import 'package:lottery_app/model/fc3d_expert.dart';
import 'package:lottery_app/predict/dlt_predict_detail.dart';
import 'package:lottery_app/predict/dlt_predict_view.dart';
import 'package:provider/provider.dart';

class ExpertDltPage extends StatefulWidget {
  @override
  ExpertDltPageState createState() => new ExpertDltPageState();
}

class ExpertDltPageState extends State<ExpertDltPage> {
  ...
  Widget build(BuildContext context) {
    return AnnotatedRegion<SystemUiOverlayStyle>(
      ...
      child: ChangeNotifierProvider<DltExpertModel>(
        create: (_) => DltExpertModel.initialize(),
        child: Scaffold(
          body: Column(
            children: <Widget>[
              NavAppBar(
  ...
  Widget _buildDltForecastList() {
    return Consumer<DltExpertModel>(
        builder: (BuildContext context, DltExpertModel model, Widget child) {
      Widget view = null;
      if (model.loaded) {
        if (model.error) {
          view = Column(
  ...
  Widget _buildForecastItem(ForecastBrief brief) {
    return GestureDetector(
      onTap: () {
        AppNavigator.push(context, DltPredictDetail(period: brief.period));
      },
      child: Container(
        padding: EdgeInsets.only(
  ...
  @override
  void didUpdateWidget(ExpertDltPage oldWidget) {

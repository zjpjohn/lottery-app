...
class DltRanksPage extends StatefulWidget {
  ///排行榜类型
  ///
  int type;

  DltRanksPage(this.type);

  @override
  DltRanksPageState createState() => new DltRanksPageState();
}

class DltRanksPageState extends State<DltRanksPage> {
  //是否加载完成
  bool _loaded = false;
            ...
            child: ListView(
              ...
              children: widget.type != 1
                  ? <Widget>[
                      ...
                      _buildRate('10码', master['red10'], 0),
                      _buildRate('20码', master['red20'], 0),
                      _buildRate('杀三', master['kill3'], 0),
                      _buildRate('蓝六', master['blue6'], 0),
  ...
  List<Widget> _getRate(var item) {
    ...
    if (item['blue6Rate'] >= 0.75) {
      views.add(MarkView(
        name: '蓝球',
      ));
    }
    if (item['blueKillRate'] >= 0.7) {
      views.add(MarkView(
        name: '杀蓝',
      ));
    }
  ...
  void _loadData() {
    ...
    params['page'] = _page;
    params['limit'] = _limit;
    request.getJson('/dlt/top/masters', params: params).then((response) {

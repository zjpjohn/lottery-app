...
class LinePainter extends LineBasePainter {
  ...
  void draw(Canvas canvas, double space, double size, double radius) {
    _paint.color = widget.activeColor;
    canvas.drawLine(Offset(index * (size + space), radius),
        Offset((index + 1) * (size + space), radius), _paint);
  }
}
...
abstract class LineBasePainter extends BasePainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double size = widget.size;
    double radius = size / 2;
    canvas.drawLine(Offset(-0.2, radius),
        Offset(widget.count * (size + space)+0.2, radius), _paint);
